...
export default function PlayScreen() {
	...
	const [deck, setDeck] = useState(deckText.cards.map(card => ({ ...card })) as Card[]);
	...
	const handleLeftSwipe = (cardIndex: number) => {
		setDeck(prevDeck => {
			...
			newDeck[cardIndex] = { ...newDeck[cardIndex], N: newDeck[cardIndex].N + 1 };
	...
	const handleRightSwipe = (cardIndex: number) => {
		setDeck(prevDeck => {
			const newDeck = [...prevDeck];
			newDeck[cardIndex] = { ...newDeck[cardIndex], Y: newDeck[cardIndex].Y + 1 };
			setCurrentCardIndex(cardIndex);
			return newDeck;
		});
